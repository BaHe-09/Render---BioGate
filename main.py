...
def crear_reporte(
    ...
    db: Session = Depends(get_db),
    # En producción deberías incluir autenticación:
    # current_user: dict = Depends(get_current_user)
):
    try:
        # Validar acceso relacionado si se proporciona
        if reporte.id_acceso_relacionado:
            acceso_existente = db.execute(
        ...
        if reporte.id_dispositivo:
            dispositivo_existente = db.execute(
...
def listar_reportes(
    estado: Optional[str] = Query(None, regex="^(Abierto|En progreso|Resuelto|Cerrado)$"),
    tipo_reporte: Optional[str] = Query(None, regex="^(Error del sistema|Fallo autenticación|Fallo de dispositivo|Acceso no autorizado|Horario irregular|Otros)$"),
    db: Session = Depends(get_db)
):
    try:
        base_query = "SELECT * FROM reportes WHERE 1=1"
        params = {}
        ...
        if tipo_reporte:
            base_query += " AND tipo_reporte = :tipo_reporte"
            params["tipo_reporte"] = tipo_reporte
            
        base_query += " ORDER BY fecha_generacion DESC"
